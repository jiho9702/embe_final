...
def calc_degree(position, center, each):
  ...
  if position == 1:
      for i in range(len(center)):
        ...
        print(each[i])
        print(d1)
        data = str(position) + ' ' + str(each[i]) + ',' + str(d1) + ';'
        print(data)
        # 전체 배열에 포함해준다
        total.append(data)
  ...
  if position == 2:
      for i in range(len(center)):
        ...
        print(each[i])
        print(d2)
        data = str(position) +  ' ' + str(each[i]) + ' ,' + str(d2) + ';'
        print(data)
        # 전체 배열에 포함해준다
        total.append(data)
...
def yolo(image_list):
  ...
  for image in image_list:
    ...
    # 중심점 저장 배열
    center = []
    results = model(source=image, conf=0.6, show=False, device='cpu')
    for r in results:
    ...
    while True:
        ...
        for t in total:
           ...
           if t[1] != '0':
              animal_count += 1
        print(animal_count)
        cnt_data = "\nanimal_number = ," + str(animal_count) + '\n'
